...
function getRestosBytypeCuisineR($typeCuisineR) {
    $resultat = array();

    try {
        $cnx = connexionPDO();
        $req = $cnx->prepare("SELECT * FROM proposer AS p JOIN resto as r ON p.idR = r.idR JOIN typecuisine as t ON p.idTC = t.idTC WHERE t.libelleTC LIKE :typeCuisineR ");
        $req->bindValue(':typeCuisineR', "%".$typeCuisineR."%", PDO::PARAM_STR);
        $req->execute();
...
if ($_SERVER["SCRIPT_FILENAME"] == __FILE__) {
    ...
    echo "gettypeCuisineR('orientale') : \n";
    print_r(getRestosByNomR("orientale"));
